...
### Alterado
- Namespace de `Hermes` → `Proxos` (todos os 52 arquivos)
- `PackageId` de `Hermes` → `Proxos`
- `PackageId` de `Hermes.Testing` → `Proxos.Testing`
- README.md atualizado com nomes de pacote corretos

### Obsoleto
- `AddHermes()` marcado como `[Obsolete]` — use `AddProxos()`. Será removido na v2.0.0.
...
- `AddHermes()` — extensão de DI (renomeada para `AddProxos()` na v1.0.2)
- MIT License
